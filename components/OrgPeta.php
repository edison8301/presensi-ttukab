...
class OrgPeta
{
        ...
        */
        ...
        foreach ($this->node as $key => $value) {
            if ($key != $this->id_kepala) {
                $this->node[$key]['y'] = $this->node[$key]['y'] + $this->lastLevelSekretariat;
            }
        }
    }
        ...
        foreach ($this->node as $key => $value)
        {
            ...
            if ($jabatan->id == $this->id_kepala) {
                $this->addLine([
                    ...
                    'y1' => $y + $this->getBoxHeight($jabatan),
                    'x2' => $x,
                    'y2' => ($value['y'] + $this->lastLevelSekretariat + 1) * ($this->getBoxHeight($jabatan) + $this->boxMarginVertical) + $this->boxMarginVertical * 0.5,
                ]);
            }
            ...
            if ($jabatan->getHasInduk() and !$jabatan->getIsStruktural()) {
                $atasan = $this->node[$jabatan->id_induk];
    ...
    public static function setNode($peta, $arrayJabatan, $level)
    {
        ...
        if ($jabatan->getIsKepala()) {
            ...
            foreach($jabatan->anakFungsional as $subjabatanArray)
            {
                $subjabatan = new JabatanPeta($subjabatanArray);

                $peta->nodeFungsional[$subjabatan->id] = [
                    'x' => 0,
                    'y' => 0,
                    'jabatan' => $subjabatan
                ];
        ...
        /**
         * Jika jabatan memiliki sub jabatan
         */
        if ($jabatan->getHasAnak()) {
            $y = $level;
            $level++;

            if ($jabatan->getIsSekretariat()) {
                $awal = $peta->lastNodeSekretariat;
                $jumlahFungsional = 0;
                ...
                foreach ($jabatan->anak as $subjabatan) {
                    $return = self::setNode($peta, $subjabatan, $level);
                    $peta = $return['peta'];
                    if ($subjabatan['jenis'] != 'Struktural') {
                        $jumlahFungsional++;
                        $level = $return['level'];
                    }
                }

                if ($peta->lastLevelSekretariat < $level) {
                    $peta->lastLevelSekretariat = $level;
                }
                //$level = $level - $jumlahFungsional;
                $akhir = $peta->lastNodeSekretariat;
                $x = 0.5 * ($awal + $akhir - 1);
                if ($jumlahFungsional != 0) {
                    $x = $akhir;
                    $peta->lastNodeSekretariat++;
                }
                $peta->nodeSekretariat[$jabatan->id] = [
                    'x' => $x,
                    'y' => $y,
                    'jabatan' => $jabatan,
                ];
        ...
        /**
         * Jika jabatan tidak memiliki sub jabatan
         */
        } else {

            /**
             * Jika jabatan merupakan struktural
             */
            if ($jabatan->getIsStruktural()) {
                $y = $level;

                if ($jabatan->getIsSekretariat()) {
                    $x = $peta->lastNodeSekretariat;
                    $peta->lastNodeSekretariat++;
                    $peta->nodeSekretariat[$jabatan->id] = [
                        'x' => $x,
                        'y' => $y,
            ...
            /**
             * Jika jabatan merupakan non struktural
             */
            } else {
                ...
                /**
                 * Jika jabatan merupakan sekretariat
                 */
                if ($jabatan->getIsSekretariat()) {
                    $x = $peta->lastNodeSekretariat;
                    if ($peta->lastLevelSekretariat < $level) {
                ...
                /**
                 * Jika jabatan merupakan NON sekretariat
                 */
                } else {

                    $x = $peta->lastNode;
                    if ($peta->lastLevel < $level) {
                        $peta->lastLevel = $level;
                    }
                    $peta->node[$jabatan->id] = [
                        'x' => $x,
                        'y' => $y,
     ...
     * @return array
     */
    public static function getArrayJabatan(Jabatan $jabatan, $params = [])
    {
        $status_kepala = false;
         ...
         *
         */
        if ($jabatan->isStruktural()==false AND $jabatan->jabatanInduk->id_induk==null) {
         ...
         * "sekretaris"
         */
        if (stripos($jabatan->nama, 'sekretaris') !== false AND $jabatan->id_induk==@$params['id_kepala']) {
         ...
         * "kepala sekretariat"
         */
        if (stripos($jabatan->nama, 'kepala sekretariat') !== false AND $jabatan->id_induk==@$params['id_kepala']) {
            $status_sekretaris = true;
            @$params['status_sekretariat'] = true;
        }
        ...
        foreach ($jabatan->findAllSub() as $subjabatan) {
            /* @var $subjabatan \app\models\Jabatan */
        ...
        $arrayJabatan = [
            ...
            'id' => $jabatan->id,
            'nama' => $jabatan->nama,
            'id_induk' => $params['status_induk'] ? null : $jabatan->id_induk,
            'jenis' => @$jabatan->getJenisJabatan(),
    ...
    public static function getArrayJabatanView(Jabatan $jabatan)
    {
        $arrayJabatan = [];
    ...
    public function saveImageOutput($filename)
    {
        $this->run();
        
        /*$filename = "temp-peta/".date('Ymdhis').".jpeg";*/
