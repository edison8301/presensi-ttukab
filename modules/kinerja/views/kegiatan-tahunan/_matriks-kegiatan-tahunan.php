...
<div style="overflow: auto; padding-bottom: 70px">
    <table class="table table-hover table-bordered" style="margin-right:20px">
        <thead>
        <tr>
        ...
        <?php if ($allKegiatanTahunanInduk === []) { ?>
            <tr>
                <td style="font-style: italic" colspan="28">
                    Belum ada skp di tahun <?= User::getTahun(); ?>
                </td>
            </tr>
        <?php } ?>
        <?php $no=1; foreach ($allKegiatanTahunanInduk as $kegiatanTahunan) {
            echo $this->render('_tr-kegiatan-tahunan',['kegiatanTahunan'=>$kegiatanTahunan,'level'=>0,'no'=>$no]);
            $i = 1;
            foreach ($kegiatanTahunan->manySub as $sub) {
                echo $this->render('_tr-kegiatan-tahunan',['kegiatanTahunan'=>$sub,'level'=>1,'no'=>null]);
                $i++;
            }
