...
<table cellpadding="6" border="1" style="background: #ffffff; margin-bottom:20px">
    ...
    <?php $i=1; foreach($allInstansiPegawai as $instansiPegawai) { ?>
        ...
        <?php
            ...
            $jumlahKotorTpp = $pegawai->getRupiahTPPSebelumPajak14($searchModel->bulan,[
                'tukin' => true,
                'penundaan' => $penundaan,
            ]);
            $jumlahKotorTpp += $pegawai->getRupiahTunjanganPlt14($searchModel->bulan);

            $kelas_jabatan = @$instansiPegawai->jabatan->kelas_jabatan;
            if(substr(@$instansiPegawai->nama_jabatan, 0, 13) == 'Tugas Belajar'
                AND $kelas_jabatan == null
            ) {
                $kelas_jabatan = 7;
            }
