            ...
            'attributes' => [
                ...
                [
                    ...
                    'value' => function ($model) use ($filter) {
                        $label = $model->getSkorIndeksIpAsn($filter->bulan);
                        
                        if (Pegawai::accessRefreshIpAsn(['bulan' => $filter->bulan])) {
                            $label .= ' ' .  Html::a('<i class="fa fa-refresh"></i>', [
                                '/tunjangan/pegawai/update-ip-asn',
                                'id' => $model->id,
                                'bulan' => $filter->bulan,
                            ], [
                                'data-toggle' => 'tooltip',
                                'title' => 'Refresh',
                                'data-confirm' => 'Yakin akan me-refresh skor IP ASN'
                            ]);
                        }
...
<div class="box box-warning">
    ...
    <div class="box-body">
        <?php $form = ActiveForm::begin([
            'layout' => 'inline',
            'action' => User::isAdmin() ? Url::to(['pegawai/view-v3', 'id' => $model->id]) : Url::to(['/tunjangan/pegawai/view-v3', 'id' => $model->id]),
            'method' => 'get',
...
<?= $this->render('_perhitungan-tpp-v3',[
    'model' => $model,
    'filter' => $filter,
    'isTampilNilaiTpp' => $isTampilNilaiTpp
]); ?>

<?= $this->render('_pembayaran-tpp-v3',[
    'model' => $model,
    'filter' => $filter,
    'isTampilNilaiTpp' => $isTampilNilaiTpp
]); ?>
